...
const HEADLINE_LINE1 = "A&C DENTAL ENT";
const HEADLINE_LINE2 = "ALLERGY CLINIC";
...
(function spawnParticles() {
  ...
  const count = isMobile ? 6 : 18; // Substantially fewer particles on mobile to improve render speed
...
// ─── 7. CARD 3D TILT ────────────────────────────────────────
// Fully bypassed on touch-screen and mobile devices to prevent gesture conflicts and lag
...
// Bypassed on mobile to prevent continuous coordinate math on every finger drag
if (window.innerWidth > 768 && !('ontouchstart' in window)) {
...
const bookings = [
  { name: 'Dr. Anooj (Dental Specialist)', desc: 'Just completed a painless wisdom tooth extraction.', initial: 'A' },
  { name: 'Dr. Chandni (ENT Specialist)', desc: 'Completed a professional clinical ear wax removal.', initial: 'C' },
  { name: 'A&C Clinic Front Desk', desc: 'Booked a new allergy patch test via WhatsApp.', initial: 'A' },
  { name: 'Dr. Florence (Dental Care)', desc: 'Completed a premium crowning and canal bridge procedure.', initial: 'F' },
  { name: 'Dr. Amal (Dental Surgeon)', desc: 'Prepared a customized dental aligner treatment plan.', initial: 'A' },
  { name: 'Assistant Keerthi', desc: 'Confirmed a contactless GPay payment option for a checkup.', initial: 'K' },
  { name: 'Support Desk Anju', desc: 'Scheduled a new pediatric foreign object removal consult.', initial: 'A' },
  { name: 'Support Desk Gauri', desc: 'Assisted a family checkup booking for allergy analysis.', initial: 'G' }
];
...
(function initHeroCardSwap() {
  ...
  // Local Debounce Helper to prevent errors with missing GSAP utils
